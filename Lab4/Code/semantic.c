...
void initHashTable() {
    ...
    //lab3 提前添加read和write函数
    SymbolTableEntry* field1 = (SymbolTableEntry*)malloc(sizeof(struct SymbolTableEntry));
    field1->name = "read";
    Type typ1 = (Type)malloc(sizeof(struct Type_));
    typ1->kind = FUNC;
    Type tyreturn = (Type)malloc(sizeof(struct Type_));
    tyreturn->kind = BASIC;
    tyreturn->u.basic = TYPE_INT;
    typ1->u.func.returnType = tyreturn;
    typ1->u.func.paramNum = 0;
    typ1->u.func.params = NULL;
    field1->type = typ1;
    insert(field1);

    SymbolTableEntry* field2 = (SymbolTableEntry*)malloc(sizeof(struct SymbolTableEntry));
    field2->name = "write";
    Type typ2 = (Type)malloc(sizeof(struct Type_));
    typ2->kind = FUNC;
    Type tyreturn2 = (Type)malloc(sizeof(struct Type_));
    tyreturn2->kind = BASIC;
    tyreturn2->u.basic = TYPE_INT;
    typ2->u.func.returnType = tyreturn2;
    typ2->u.func.paramNum = 1;
    FieldList write_param = (FieldList)malloc(sizeof(struct FieldList_));
    write_param->name = "write_param_name";
    Type write_param_typ = (Type)malloc(sizeof(struct Type_));
    write_param_typ->kind = BASIC;
    write_param_typ->u.basic = TYPE_INT;
    write_param->type = write_param_typ;
    typ2->u.func.params = write_param;
    field2->type = typ2;
    insert(field2);
}
